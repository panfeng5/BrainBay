...
void draw_speller(SPELLEROBJ * st)
{
	...
	if (st->dictfile[0])
	{
		...
		sprintf(szdata,"W�rterbuch: %s, %d words.",fn, st->wordcount);
	  ...
	  void SPELLEROBJ::learn_words()
	  {
		  ...
		  { 
			if ( ((word[x]>='a') && (word[x]<='z')) || ((word[x]>='A') && (word[x]<='Z'))
				|| (word[x]=='�') || (word[x]=='�') || (word[x]=='�') || (word[x]=='�') || (word[x]=='�') || (word[x]=='�'))
			{
				actword[a++]=word[x];
				actword[a]=0;
			}
